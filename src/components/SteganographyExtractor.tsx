...
export function SteganographyExtractor() {
  ...
  const handleExtractFile = async () => {
    ...
    try {
      ...
      const mockExtractedFile = {
        ...
        size: 245760, // ~240 KB
        blob: new Blob(["Mock extracted file content"], {
      ...
      setState((prev) => ({
        ...
        result: {
          success: false,
          message:
            "Failed to extract file. Please check your password and try again.",
        },
      }));
    }
  };
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Select the MP3 file that contains hidden data
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    <p className="text-lg font-medium text-muted-foreground">
                      Drop your stego audio file here
                    </p>
                    <p className="text-sm text-muted-foreground">
                      or click to browse
                    </p>
            ...
            />
            {state.stegoFile && (
              <AudioPlayer file={state.stegoFile} title="Stego Audio" />
            )}
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Enter the same password used to hide the file
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-2">
            ...
            />
            <p className="text-sm text-muted-foreground">
              This must match the password used when hiding the file
            </p>
          </div>
        </CardContent>
      </Card>
                    ...
                    <CardContent className="pt-4">
                      <div className="flex items-center gap-3">
                        <div className="text-2xl">
                          {getFileIcon(state.result.extractedFile.type)}
                        </div>
                        <div className="flex-1">
                          <p className="font-medium text-green-800">
                            {state.result.extractedFile.name}
                          </p>
                          <p className="text-sm text-green-600">
                            {(state.result.extractedFile.size / 1024).toFixed(
                              2
                            )}{" "}
                            KB • {state.result.extractedFile.type}
                          </p>
                        </div>
                        <Button
                  ...
                  {state.result.metadata && (
                    <div className="grid md:grid-cols-2 gap-4 text-sm">
                      <div className="flex items-center gap-2 text-green-700">
                        ...
                        <span>
                          Original name: {state.result.metadata.originalName}
                        </span>
                      </div>
                      <div className="flex items-center gap-2 text-green-700">
      ...
      <Card className="bg-muted/50">
        ...
        <CardContent className="text-sm space-y-2">
          <p>
            • Make sure you're using the exact same password that was used to
            hide the file
          </p>
          <p>
            • The stego audio file should be the output from the "Hide Files"
            process
          </p>
          <p>
            • If extraction fails, verify the file hasn't been compressed or
            modified
          </p>
          <p>
            • Large files may take longer to extract depending on the LSB
            settings used
          </p>
        </CardContent>
      </Card>
